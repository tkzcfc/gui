...
function read_file(filename)
  ...
  if utils.detectStringCode(str) == "UTF-8" then
      str = utils.utf8_to_ansi(str)
  end
...
function write_file(filename, str)
  ...
  local toCode = utils.detectFileCode(filename)
  if toCode == "unknown" or toCode == "ASCII" then
    toCode = "UTF-8"
  end

  if toCode == "UTF-8" then
      str = utils.ansi_to_utf8(str)
  else
      local fromCode = utils.detectStringCode(str)

      print("from code", fromCode)
      print("to code", toCode)

      str = utils.convertStringEncoding(str, fromCode, toCode)
      if not str then
        iup.Message("Error", "convert codeing fail: " .. fromCode .. " to " .. toCode)    
        return false
      end
