...
static std::wstring StringUtf8ToWideChar(const std::string& strUtf8)
{
	...
	if (!strUtf8.empty())
	{
		...
		{
			spdlog::error("Wrong convert to WideChar code {}", GetLastError());
		}
	}
	return ret;
}
...
static std::string UTF8StringToMultiByte(const std::string& strUtf8)
{
	...
	if (!strUtf8.empty())
	{
		...
		{
			spdlog::error("Wrong convert to Ansi code {}", GetLastError());
		}
	}
	...
	if ((GetFileAttributesW(path.c_str())) == INVALID_FILE_ATTRIBUTES)
	{
		...
		for (size_t i = 0, size = dirs.size(); i < size; ++i)
		{
			...
			if (!isDirectoryExist(utf8Path))
			{
				...
				{
					spdlog::error("Fail create directory '{}' , error code is {}", utf8Path, GetLastError());
					return false;
				}
			}
		}
...
bool GFileSystem::removeFile(const std::string &filepath)
{
	...
	{
		spdlog::error("Fail remove file '{}', error code is ", filepath, GetLastError());
		return false;
	}
}
...
bool GFileSystem::renameFile(const std::string &oldfullpath, const std::string &newfullpath)
{
	...
	if (isFileExist(newfullpath))
	{
		...
		{
			spdlog::error("Fail to delete file {}, error code is {}", newfullpath, GetLastError());
		}
	}
	...
	{
		spdlog::error("Fail to rename file '{}' to '{}', error code is {}", oldfullpath, newfullpath, GetLastError());
		return false;
	}
}
...
uint8_t* GFileSystem::readFile(const std::string& filename, uint32_t& outSize)
{
	...
	if (!successed) 
	{
		spdlog::error("Get data from file '{}' failed, error code is {}", filename, GetLastError());
		free(buffer);
		// ReadFailed
		return NULL;
	}
...
{
	std::string content;
	readStringFromFileEx(filename, content);
	return content;
}

bool GFileSystem::readStringFromFileEx(const std::string& filename, std::string& outData)
{
	uint32_t len = 0;
